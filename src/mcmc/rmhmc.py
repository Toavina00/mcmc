...
def sample(
    ...
    @jax.jit
    def __hamiltonian(x, p, GL, Gdet, Gjac, GidG):
    ...
    Gx, GL, Gdet, Gjac, GidG = __riemann_metric(x_init)

    def fp_p(carry, _):
        x, p, dH, Gx, GL, Gdet, Gjac, GidG = carry
        p_new = p - 0.5 * eps * dH
        _, dH = __hamiltonian(x, p_new, GL, Gdet, Gjac, GidG)
        return (x, p, dH, Gx, GL, Gdet, Gjac, GidG), p_new

    def fp_x(carry, _):
        x, p, w, w_new, _, _, _, _, _ = carry
        x_new = x + 0.5 * eps * (w + w_new)
        Gx, GL, Gdet, Gjac, GidG = __riemann_metric(x_new)
    ...
    def leapfrog(carry, _):
        ...
        x = fp_arr_x[-1]
        _, dH = __hamiltonian(x, p, GL, Gdet, Gjac, GidG)
        p = p - 0.5 * eps * dH
        H, dH = __hamiltonian(x, p, GL, Gdet, Gjac, GidG)
    ...
    def _loop(carry, _):
        ...
        return (key, new_rej, new_x, new_Gx, new_GL, new_Gdet, new_Gjac, new_GidG), output
